...
const GpuMonitor: React.FC = () => {
  ...
  const content = useMemo(() => {
    ...
    if (!gpuData.hasAdlx) {
      return (
        <div className="bg-yellow-900 border border-yellow-700 text-yellow-300 px-4 py-3 rounded relative" role="alert">
          <strong className="font-bold">ADLX not available!</strong>
          <span className="block sm:inline"> ADLX helper or driver is not available.</span>
    ...
    if (gpuData.gpus.length === 0) {
      return (
        <div className="bg-yellow-900 border border-yellow-700 text-yellow-300 px-4 py-3 rounded relative" role="alert">
          <span className="block sm:inline">No GPUs found, but ADLX is available.</span>
        </div>
      );
    }
